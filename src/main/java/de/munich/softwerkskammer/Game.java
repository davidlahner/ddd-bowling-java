...
public class Game {
	...
	private int score(int beginFrame, int endFrame) {
		...
		for (int frame = beginFrame; frame <= endFrame; frame++) {
			if (isStrike(frameIndex)) {
				if (frameIndex != 9
						&& (frameIndex + 1 == endFrame
								&& (rolls[frameIndex + 1] == NOT_ENOUGH_INFORMATION || rolls[frameIndex + 2] == NOT_ENOUGH_INFORMATION) //
						|| frameIndex + 2 == endFrame
								&& rolls[frameIndex + 2] == NOT_ENOUGH_INFORMATION)) {
	...
	private int sumOfBallsInFrame(int frameIndex) {
		int firstRoll = rolls[frameIndex] == NOT_ENOUGH_INFORMATION ? 0
				: rolls[frameIndex];
		int secondRoll = rolls[frameIndex + 1] == NOT_ENOUGH_INFORMATION ? 0
				: rolls[frameIndex + 1];
